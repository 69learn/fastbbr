...
installbbr(){
	...
	stty erase '^H' && read -p "After needing to restart the VPS，Only then can the BBR/BBR magic modified version be turned on，Do you want to restart now ? [Y/n] :" yn
	[ -z "${yn}" ] && yn="y"
	if [[ $yn == [Yy] ]]; then
		echo -e "${Info} VPS 重启中..."
		reboot
...
start_menu(){
clear
echo && echo -e " FAST BBR script ${Red_font_prefix}[v${sh_ver}]${Font_color_suffix}
  -- 69 LEARN --
  
 ${Green_font_prefix}0.${Font_color_suffix} Upgrade script
————————————Kernel Management————————————
 ${Green_font_prefix}1.${Font_color_suffix} Install BBR/BBR modified version of the kernel
 ${Green_font_prefix}2.${Font_color_suffix} Install BBRplus version kernel
...
————————————Accelerate Management————————————
 ...
 ${Green_font_prefix}5.${Font_color_suffix} Use BBR magic modified version to accelerate
 ${Green_font_prefix}6.${Font_color_suffix} Use violent BBR magic modified version to accelerate (some systems are not supported)
 ${Green_font_prefix}7.${Font_color_suffix} Use BBRplus version to accelerate
 ${Green_font_prefix}8.${Font_color_suffix} Use Lotserver to accelerate
...
————————————————————————————————" && echo
	...
	if [[ ${kernel_status} == "noinstall" ]]; then
		echo -e " Current status: ${Green_font_prefix}is not installed${Font_color_suffix} Acceleration core ${Red_font_prefix}请先安装内核${Font_color_suffix}"
	else
